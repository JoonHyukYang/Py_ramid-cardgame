...
class Drawpile:
    ...
    def open_drawing(self,root):
        tmp = ImageTk.PhotoImage(Image.open(self.get.cardImage))
        if self.drawpile != []:
...
class Pyramid(Drawpile):
    ...
    def newpy(self):
        ...
        for i in range(7):
            ...
            for j in range(i+1):
                ...
                if i != 6:
                    self.__py[i].append(Checkbutton(self.root,image=p, state=DISABLED, 
                        variable = 1, selectimage=self.b))
                    self.nn(self.__py,i,j)
                    self.__py[i][j].select()
                else:
                    self.__py[i].append([Checkbutton(self.root,image=p, 
                        selectimage=self.b), tmp.value])
                    self.nn(self.__py,i,j)
                    self.__py[i][j].deselect()
                self.__py[i][j].image = p
                self.__py[i][j].selectimage = self.b
                

    def nn(self, ll, i,j):
        ll[i][j]["command"] = lambda: self.isselect(i,j)
    ...
    def isselect(self, x, y):
        if 0 <= x <= 6 and 0 <= y <= x and self.py[x][y] != None:
            if x == 6:
                return True
    ...
    def draw(self, root):
        if self.pile != []:
            d = Button(root, command = lambda: self.open_drawing(root), image=self.b)
            d.image = self.b
            d.place(x =140 ,y=500)
