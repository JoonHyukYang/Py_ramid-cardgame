    ...
    def newpy(self):
        ...
        for i in range(7):
            ...
            for j in range(i+1):
                ...
                if i != 6:
                    self.__py[i].append(Checkbutton(self.master,image=p, width="73", height="98", state=DISABLED, selectimage=self.b, command = lambda: self.value(t[i*(i+1)])))
                else:
                    self.__py[i].append(Checkbutton(self.master,image=p, width="73", height="98", selectimage=self.b, command = lambda: self.value(t)))
